...
<div>
	...
	List<model.Student> student = q.list();
	%>
	
<form action ="viewstudent" method ="post">
	<table id="t2" align="center">
		<tr>
			<td align="center">Select Student
		<tr>
			<td align="center"><select name="stuid" required>
				<option value="" disabled selected hidden>Student</option>
					<% for(model.Student s: student){%>
					<option value="<%=s.getSroll()%>">[ID:<%=s.getSroll()%>] <%=s.getSname()%></option><%}%>
				</select>
		<tr>
